...
function checkUid($conn, $username, $email, $phone){
  $sql = "SELECT * FROM shops WHERE userName = ? OR email = ? OR phone = ?;";
...
function  createUser($conn, $fullname, $email, $username, $phone, $age, $nationality, $category) {
  $sql = "INSERT INTO shops (fullName, email, userName, phone, age, nationality, category) VALUES (?, ?, ?, ?, ?, ?, ?);";
  $stmt = mysqli_stmt_init($conn);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    header("location: ../index.php?error=stmtfailed");
    exit();
  ...
  mysqli_stmt_bind_param($stmt, "sssssss", $fullname, $email, $username, $phone, $age, $nationality, $category);
  mysqli_stmt_execute($stmt);
  mysqli_stmt_close($stmt);
...
function uploadAds($conn, $adsName, $adsDes, $adsCat, $user, $adsclass, $price){
  $adsId = uniqid('Jiads', true);
      $sql = "INSERT INTO prds (AdsName,userName, Adsdes, AdsCat, Adssn, class, price) VALUES (?, ?, ?, ?, ?, ?, ?);";
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
...
function login($conn, $username, $pwd){
  ...
  if ($checkpwd === false) {
    ...
    session_start();
    $_SESSION['owner'] = $uidExist['userName'];
  ...
  function change_password($conn, $newpwd, $Cuser){
    $pwd = password_hash($newpwd, PASSWORD_DEFAULT);
    $sql = "UPDATE users SET pwd = '".$pwd."' WHERE userName = '".$Cuser."'";
...
function ChangeDetails($conn, $busname, $fullname, $state, $email, $phone, $CbusN){
  if (!checkUid($conn, $busname, $email, $phone)) {
    return false;
    # code...
  }
    $sql = "UPDATE users SET fullName = ?, email = ?, userName = ?, nationality = ?, phone = ? WHERE userName = ? ;";
  $stmt = mysqli_stmt_init($conn);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    return false;
  }
  ...
  mysqli_stmt_bind_param($stmt, "ssssss", $fullname, $email, $busname, $state, $phone, $CbusN);
  mysqli_stmt_execute($stmt);
  mysqli_stmt_close($stmt);
  //  header("location: index.php?error=none");
  //  exit();
